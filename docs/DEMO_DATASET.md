...
开发环境 Web 顶部的身份选择器可直接切换上述 8 个核心 Full Demo 账号。浏览器仅提交固定的 `platform_user_id`，开发 BFF 从服务端白名单解析对应的 `WEB` 平台类型；原有 `TEST_PLATFORM / test-user-*` 测试账号仍可继续使用。角色、楼宇范围和权限均以 `/api/v1/users/me` 的 Backend 返回为准。

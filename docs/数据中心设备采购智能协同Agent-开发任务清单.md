...
> 建立日期：2026-08-04
> 最近更新：2026-08-14
> 当前阶段：阶段 23、阶段 24 已完成；阶段 21 的生产化部署与最终交付收尾仍进行中
> 当前需求与设计文档：需求分析 V2.2、技术方案 V1.2、功能模块划分 V1.2、系统分析 V1.2；开发计划 V1.1 为 2026-08-04 历史规划快照
...
| DEV-04 | M5 `RAG-000` 至 `RAG-006`、`KNW-001` 至 `KNW-003` | 真实知识文档导入、权限检索、引用、知识/混合问答和评测 | 开始前必须确认真实知识材料 | `[x]` 2026-08-07，阶段 12–16 完成 Qdrant 混合 RAG |
| DEV-05 | M6 `ANB-001` 至 `ANB-005`、`RSK-001` 至 `RSK-003`、`SUP-001`、`CAS-001` | 查询、统计、风险、履约、案例接口和后端契约 | 实现风险规则前确认延期、部分入库等统计口径 | `[x]` 2026-08-05 |
| DEV-06 | M7 `MCP-008`、`PLN-001`、`EXE-001` 至 `EXE-002`、`ANA-001` 至 `ANA-004`、`MEM-002` | Analysis Agent、Planner/Executor、连续追问、结构化结果和标准答案评测 | 真实模型联调前确认供应商、模型和密钥 | `[x]` 2026-08-10，真实模型 Planner/Compose 验收完成 |
| DEV-07 | M8 `INV-001` 至 `INV-004`、`REV-001` 至 `REV-003` | 审批风险调查、证据校验、Review 和评测 | 发现新增审批业务口径时确认 | `[x]` 2026-08-10，真实 Review 与风险链路已验收；补查循环仍为后续优化 |
| DEV-08 | M9 `WEB-001` 至 `WEB-006` | Web 智能协同入口、引用/表格/风险展示和前端安全测试 | 需要改变既有采购页面主流程时确认 | `[x]` 2026-08-11，React/SSE/HITL/Context Assistant 完成 |
| DEV-09 | M10 `TRC-002` 至 `TRC-003`、`EVL-001` 至 `EVL-004`、`WEB-007` | 全链路 Trace、评测、回归基线和执行详情 | 更新评测基线前确认 | `[x]` 2026-08-14，固定 Agent acceptance 与阶段耗时已实现 |
| DEV-10 | M11 `RES-001` 至 `RES-003`、`SEC-001`、`DEP-001` 至 `DEP-002`、`DEM-001` 至 `DEM-003`、`DOC-001`、`REL-001` | 容错、安全、部署、三条演示、文档和最终验收 | 破坏性资源操作或外部发布前确认 | `[-]` 模型/RAG/Agent 主链已完成；frontend 容器、备份恢复和最终发布仍待收尾 |
...
| M5 | 完成基础 RAG（历史规划） | 文档导入、Metadata、引用、Knowledge Agent；Chroma 选型已由阶段 13 的 Qdrant 决策替代 | `[x]` 2026-08-07，阶段 12–16 完成 |
| M6 | 补齐分析型后端接口 | 查询 DSL、风险信号、履约统计、相似案例 | `[x]` 2026-08-05 |
| M7 | 完成复杂查询分析 | Planner/Executor、多工具调用、连续追问、表格结果 | `[x]` 2026-08-10 |
| M8 | 完成审批风险调查 | 风险补查、证据摘要、Review | `[x]` 2026-08-10，补查循环保留为后续优化 |
| M9 | 完成 Web 智能协同入口 | 对话、表格、引用、风险摘要和任务状态 | `[x]` 2026-08-11 |
| M10 | 完成 Trace 与评测 | 节点 Trace、评测集、回归检测 | `[x]` 2026-08-14 |
| M11 | 完成容错、部署和验收 | 重试、熔断、降级、Compose、三条演示 | `[-]` Backend/Agent Compose 已有；Web 生产部署与最终交付待收尾 |
...
| RAG-002 | 定义文档 Metadata Schema | RAG-001 | 包含版本、生效时间、角色、设备、权限和过期状态 | `[x]` 2026-08-07 |
| RAG-003 | 实现文档解析、章节切分和原文位置保留 | RAG-002 | Chunk 可追溯到文档和章节 | `[x]` 2026-08-07 |
| RAG-004 | 建立向量索引和基础向量检索 | RAG-003 | 可重复导入、更新和重建；历史 Chroma 选型已由 Qdrant 替代 | `[x]` 2026-08-07 |
| RAG-005 | 实现权限过滤和结构化引用 | RAG-004 | 越权、过期文档不进入回答证据 | `[x]` 2026-08-07 |
| KNW-001 | 实现 Knowledge Agent | RAG-005 | 无依据时明确无法确认 | `[x]` 2026-08-07 |
| KNW-002 | 完成纯知识问答场景 | KNW-001 | 回答包含正确文档、版本和章节 | `[x]` 2026-08-07 |
| KNW-003 | 完成“为什么当前申请不能提交”混合问答 | KNW-001, MCP-007 | 同时使用文档和实时后端数据 | `[x]` 2026-08-10，真实 RAG + Tool 验收 |
| RAG-006 | 建立基础 RAG 评测集 | KNW-002, KNW-003 | Top-K、引用和忠实度可评测 | `[x]` 2026-08-07 |
...
| ANA-002 | 完成自然语言转查询 DSL | ANA-001 | 条件提取完整；Schema 失败有限重试 | `[x]` 2026-08-10，真实模型 Planner 验收 |
...
| INV-002 | 并行补查价格、履约、案例和制度 | INV-001, KNW-001 | 各证据保留来源和失败状态 | `[x]` 2026-08-10，Tool 与知识证据均已接入 |
| INV-003 | 生成结构化风险摘要 | INV-002 | 每项包含事实、规则、可能原因和人工确认项 | `[x]` 2026-08-05（确定性摘要） |
| REV-001 | 实现程序证据与数字校验 | INV-003 | 数字和记录编号可追溯 | `[x]` 2026-08-05 |
| REV-002 | 实现 Review Agent 语义审查 | REV-001 | 检查遗漏、编造、推测和审批越权 | `[x]` 2026-08-10，真实模型与确定性回退均已验证 |
| REV-003 | 实现一次补查或重写回路 | REV-002 | 循环次数和总预算受限 | `[-]` `revised_answer` 可强制采用；独立补查循环尚未实现，保留为优化项 |
| INV-004 | 建立风险摘要评测集 | REV-003 | 风险召回、错误风险、证据覆盖和越权可评测 | `[x]` 2026-08-14，风险固定集与 Agent acceptance 已覆盖 |
...
| WEB-003 | 实现引用和知识回答展示 | KNW-003, WEB-002 | 文档、版本和章节可见 | `[x]` 2026-08-11，SSE Citation 与来源展示完成 |
...
| ID | 任务 | 依赖 | 验收方式 | 状态 |
|---|---|---|---|---|
| TRC-002 | 完成 Graph、RAG、MCP 和 Review 全链路 Trace | REV-003 | 一个 Trace 可还原完整执行链 | `[x]` 2026-08-11 |
| TRC-003 | 记录模型、Token、费用、步骤和耗时 | TRC-002 | 指标可按请求汇总 | `[x]` 2026-08-14；只接受 Provider 报告用量，缺失值不估算 |
| EVL-001 | 建立 Router 评测集 | RTR-001 | 分类和复合任务拆分可重复评测 | `[x]` 2026-08-06 |
| EVL-002 | 建立 Tool 参数和越权评测 | MCP-008 | 错误工具及越权调用直接失败 | `[x]` 2026-08-06 |
| EVL-003 | 汇总 RAG、复杂查询和风险评测 | RAG-006, ANA-004, INV-004 | 生成统一报告 | `[x]` 2026-08-14，固定集与汇总 Runner 已实现 |
...
| RES-003 | 实现 RAG、工具、模型和 Review 降级 | RES-001 | 不完整分析不会描述为完整结论 | `[x]` 2026-08-10 |
| SEC-001 | 完成 Prompt Injection 和工具权限测试 | RES-003 | 文档内容不能触发越权工具或泄露敏感数据 | `[x]` 2026-08-11 |
| DEP-001 | 扩展 Docker Compose | WEB-006, RES-003 | 后端 8000、Agent 8100、stdio MCP、MySQL、Redis、Qdrant 可启动 | `[x]` 2026-08-10；React frontend 未纳入 Compose |
| DEP-002 | 增加 Agent 镜像、健康检查和配置示例 | DEP-001 | 非 root 运行；密钥不进入镜像 | `[x]` 2026-08-06 |
| DEM-001 | 固化知识与业务混合问答演示 | DEP-002 | TEST 数据下重复运行稳定 | `[x]` 2026-08-10，真实 RAG + Tool 闭环完成 |
| DEM-002 | 固化复杂查询与连续追问演示 | DEP-002 | 结果与标准答案一致 | `[x]` 2026-08-06 |
| DEM-003 | 固化审批风险调查演示 | DEP-002 | 每条风险有证据且不替代审批 | `[x]` 2026-08-06 |
| DOC-001 | 更新 README、架构、接口、部署和演示文档 | DEM-001 至 DEM-003 | 文档与代码和 Compose 一致 | `[x]` 2026-08-14，当前文档一致性整理完成 |
| REL-001 | 执行最终回归和验收 | DOC-001 | 三条演示、全部测试、Ruff、权限和恢复验收通过 | `[-]` 模型/RAG/Web 主链已验收；生产 Web 部署、备份恢复与发布流程待完成 |
...
- **主要工作**：在不改变 Agent 业务路由和安全边界的前提下，减少简单请求的不必要模型步骤，优化
  Router/Query Rewrite/Compose/Review 的输出预算与 thinking 配置，引入 Rewrite/Retrieval/Embedding
  Cache，记录模型、检索、重排、Tool 和持久化阶段耗时；补齐固定 Agent acceptance 与轻量 RAG
  acceptance。
- **实际实现**：RAG Provider 同时支持默认 `aliyun_bailian` API 和可选 `local_bge`；默认模型为
  `qwen3.7-text-embedding`/`qwen3-rerank`。简单 Knowledge、Realtime 与 Form Prefill 不强制进入
  Planner，模型 Review 仅用于 Hybrid、Risk 及确需模型 Planner 的复杂查询。Role 级输出 Token 上限和
  thinking 开关由 `PERFORMANCE_OPTIMIZATIONS_ENABLED` 控制。
- **固定数据集**：`agent_acceptance_evaluation_v0.1.json` 共 25 条，Knowledge、Realtime、Complex
  Query、Risk Investigation、Form Prefill 各 5 条；`rag_acceptance_evaluation_v0.1.json` 共 10 条。
- **评测输出**：任务成功率、路由准确率、工具准确率、平均/P50/P95 耗时、成功模型调用数、Fallback、
  Tool 调用数、错误、性能字段和 model/retrieval/rerank/tool 阶段耗时；动态采购单 ID 由运行脚本从
  当前演示数据解析，不把固定数据库主键写成永久前提。
- **脚本与测试**：`scripts/run_agent_acceptance_evaluation.py`、`scripts/benchmark_agent_latency.py`、
  `scripts/benchmark_rag_api.py`、`scripts/benchmark_rag_cpu.py`，以及对应 evaluation/performance 单测。
- **当前状态**：`[x]` 2026-08-14。这里记录框架和覆盖范围，不把某次受模型、网络和数据影响的临时
  数值固化为永久质量承诺。
...
1. 阶段 23 和阶段 24 已完成。当前真正未收尾的是生产化 Web 部署：`compose.yaml` 没有 frontend
   service，Dockerfile 也不执行 Node 构建；需要明确静态托管、反向代理和生产身份层方案。
2. 阶段 21 仍保留备份/恢复演练、发布流程、安全检查汇总和最终交付验收；现有 Compose 已覆盖
   MySQL、Redis、Qdrant、migration、Backend 和 Agent，不应再写成“等待模型/RAG”。
3. 真实主模型、Fallback、Embedding、Reranker、7 文档 Qdrant 索引和知识/实时/混合/复杂/风险/
   预填路径均已有实现与验证入口；后续重点是持续观察远程超时、Fallback 偶发空结构化正文和性能回归。
4. Review 当前支持结构化语义审查、确定性回退和 `revised_answer` 强制采用，但没有独立的“补查后
   再循环”Graph 分支；仅在确有业务价值时作为后续优化，不影响当前安全降级。
